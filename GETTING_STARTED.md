...
This file will outline the project setup process for new developers working on this project. Development support is possible on any operating system that can run Microsoft .NET 9.0 SDK and Node.js. (windows, linux, mac supported)
...
- Database: The database is a MySQL server that is hosted remotely. The backend will access it to get data. To set up your own database to match the one used in production, execute the database creation script `init_database.sql` (intended for MySQL) `
